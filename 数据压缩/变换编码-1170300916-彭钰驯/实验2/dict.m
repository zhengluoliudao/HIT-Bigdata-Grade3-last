...
close all;clear;clc;
%将原始图像转换为灰度图像
I1=imread('D:\lena.bmp');%读取图像
figure(1);imshow(I1);title('原始图像');
I=im2double(I1);%变换前的原始数据
...
figure;imshow(I3);title('DCT量化后的频域图像');%显示DCT变换后的频域图像
%分块做DCT反变换（8×8）
for i=1:8:505
    for j=1:8:505
        Q = k.*a1;
        P=I3(i:i+7,j:j+7).* Q;%反量化
        K=C'*P*C;
        I4(i:i+7,j:j+7) = K;
    end
end
...
imwrite(I4,'D:\result.jpg');

%计算两幅图像的峰值信噪比
f1 = imread('D:\lena512.bmp');
f2 = imread('D:\result.jpg');
